...
// Full screen mode listener. Without that JS dont have information about fullscreen mode enter.
...
function checkEdit(){
	if(editFirst){
		getNode("textEntry").style.background = 'white';
		editFirst=0;
	}
}
...
function hexToStr(hex){
	...
	var st=bin2String(buf);
    return st; //return the contents of the cell;
}
function hexToString(cell, formatterParams, onRendered){
	//cell - the cell component
    //formatterParams - parameters set for the column
    //onRendered - function to call when the formatter has been rendered
	var buf=hexToBytes(cell.getValue());
	var st=bin2String(buf);
    return hexToStr(cell.getValue()); //return the contents of the cell;
}
function hexToInt(cell, formatterParams, onRendered){
	//cell - the cell component
    //formatterParams - parameters set for the column
    //onRendered - function to call when the formatter has been rendered
    return parseInt(cell.getValue(),16); //return the contents of the cell;
}
function trimValue(cell, formatterParams, onRendered){
	//cell - the cell component
    //formatterParams - parameters set for the column
    //onRendered - function to call when the formatter has been rendered
	var s=cell.getValue();
	if(s){
	    return s.substr(0,64); //return the contents of the cell;
	}else{
		return "";
	}    
}
function countIntValue(cell, formatterParams, onRendered){
	//cell - the cell component
    //formatterParams - parameters set for the column
    //onRendered - function to call when the formatter has been rendered
	var s=cell.getValue();
	if(s){
		return s.length/8+" records";
	}else{
		return "0 records";
	}	
}

function idVert(cell, formatterParams, onRendered){
		recordID=hexToStr(cell.getValue());
		
		var id=recordID.replace(/[\[\]]*/g,"");
		id=id.replace(/ID:/,"");
		id=id.replace(/(.)/g,"$1<br/>");
		var idNode=getNode("menuRecordId");
		idNode.innerHTML=id;
		return '';
}

function linkCategory(cell, formatterParams, onRendered){
	//cell - the cell component
    //formatterParams - parameters set for the column
    //onRendered - function to call when the formatter has been rendered
	var s=cell.getValue();
	if(s){
		return '<button class="category" onClick="location.href=\'/renderCategory.esp?tag='+encodeURL(s)+'\';">'+s+'</button>';
	}else{
		return "";
	}	
}
function printID(cell, formatterParams, onRendered){
	var buf=hexToBytes(cell.getValue());
	var st=bin2String(buf);
	st=st.replace(/.*\[\[ID:([^\]]*).*/,"$1");
	return st;
}
function getDictName(cell, formatterParams, onRendered){
	var st=cell.getValue();
	if(st.indexOf(':')==-1){
		var buf=hexToBytes(st);
		st=bin2String(buf);
		st=st.replace(/.*Category:Dictionary__([^\]]*).*/,"$1]");
		st=st.replace(/[\[\]]*/g,"");
		return st;
	}else{
		return "CTL";
...
function getTitle(cell, formatterParams, onRendered){
	var buf=hexToBytes(cell.getValue());
	var st=bin2String(buf);
	st=st.replace(/.*\[\[Title:([^\]]*).*/,"$1");
	return st;
}
function getTitleURL(cell, formatterParams, onRendered){
	var buf=hexToBytes(cell.getValue());
	var text=bin2String(buf);
	st=text.replace(/.*\[\[Title:([^\]]*).*/,"$1");
	var id=text.replace(/.*\[\[ID:([^\]]*).*/,"$1");
	var st_=st.substr(0,76);
	if(st_!=st)st_+="...";
	st='<button class="category" onClick="location.href =\'/renderCategory.esp?id='+id+'\';">'+st_+'</button>';
	return st;
}
function dictionaryRecord(cell, formatterParams, onRendered){
	var st=cell.getValue();
	if(st.indexOf(':')==-1){
		var buf=hexToBytes(st);
		st=bin2String(buf);
		var Title=st.replace(/.*\[\[Title:([^\]]*).*/,"$1");
		//console.log(st);
	
		var record=st.replace(/\[\[/g,"<");
		record=record.replace(/\]\]/g,">");
		record=record.replace(/<[^>]*>/g,"");
		record=record.replace(/¶/g," ");
			var element=cell.getElement();
		element.style.padding=0;
		return '<div class="printDict"><div class="dictEntry">'+Title+" <font color='#cccccc'>‣</font> "+record+'</div></div>';
	}else{
	
		return '<div class="printDict"><div class="dictEntry">'+st+'</div></div>';
	
	}		
		
}
function renderRecordWiki(cell, formatterParams, onRendered){
	var hex=cell.getValue();
	var buf=hexToBytes(hex);
	var st=bin2String(buf);
	...
	if(pageTitle==""){
		var Title=st.replace(/.*\[\[Title:([^\]]*).*/,"$1");
		//var titleHolder=getNode("pageTitle");
		//titleHolder.innerHTML=Title;
		//pageTitle=Title;
		pageContent=st;
		recordID=st.replace(/.*\[\[ID:([^\]]*).*/,"$1");
		var id=recordID.replace(/(.)/g,"$1<br/>");
		getNode("pageMenu").innerHTML+=id;		
	}	
	//set page cover record
	if(!bookCover.length){
		bookCover=st.replace(/\]\][^\[]*\[\[/gm,"\"},\n{\"p1\":\"");
		bookCover=bookCover.replace(/^[^\[]*\[\[/m,"[{\"p1\":\"");
		bookCover=bookCover.replace(/\]\][^\[]*$/m,"\"}]");
		bookCover=JSON.parse(bookCover);
		print(bookCover);
		table2.replaceData(bookCover);
	}
	
	
	var report=st.replace(/\[\[/g,"<");
	...
	var element=cell.getElement();
	element.style.padding=0;
	return '<div class="printPage"><div class="pageEdit" id="pageEdit">'+report+'</div></div>';
}

function  renderCatalogRecord(cell, formatterParams, onRendered){
...
function  renderCatalogRecordURL(cell, formatterParams, onRendered){
	
	var hex=cell.getValue();
	var buf=hexToBytes(hex);
	var st=bin2String(buf);
	
	var id=st.replace(/.*\[\[ID:([^\]]*).*/,"$1");
	...
	var st_=report.substr(0,176);
	if(st_!=st)st_+="...";
	report='<div class="category" onClick="location.href =\'/renderCategory.esp?id='+id+'\';">'+st_+'</div>';
...
///Callback functions
function openRecord(e, row){
	var id=row.getData().p0;
	id=hexToStr(id);
	id=id.replace(/[\[\]]*/g,"");
	id=id.replace(/ID:/g,"");
	location.href ='/renderCategory.esp?id='+id;
}
;(function (global, factory) {
	...
	var formatReport=function (data) {
		...
		var report="";
		data=data.trim();  //end \n\n\n\n\n\n from POST data
		var lines =data.split("^_^");
		for(var i=0;i<lines.length;i++){
			if(i==0){
				report+='{"last_page":'+lines[i]+',"data":[';
			}else{
				if(!lines[i])continue;
				if(i!=1)report+=',';
				report+='{';
	...
	ColumnManager.prototype.createHeadersElement=function () {
		...
		if(this.table.options.noHeader){
			el.class="";
			el.style.height="0px";
		}else{
	...
	ColumnManager.prototype._verticalAlignHeaders=function () {
		...
		if(minHeight<self.table.options.columnTitleHeight)minHeight=self.table.options.columnTitleHeight;  
		self.columns.forEach(function (column) {
	...
	Column.prototype._buildColumnHeader=function () {
		...
		if (def.cssClass) {
			var sp=def.cssClass.split(" ");
			self.element.classList.add(sp[1]);
		}
		if (def.field) {
	...
	Row.prototype.normalizeHeight=function (force) {
		...
		this.calcHeight();
		if(this.table.options.fitToScreen){
			//////////////////////////
			//the book pages read mode
			
			//if(this.table.options.paginationSize==1){
				//## adjust table height by screen in case of page edit (one-row table)
				var screenH=document.documentElement.clientHeight-this.table.options.topOffset;
				var header=this.table.element.childNodes[0].offsetHeight;
				var footer=this.table.element.childNodes[2].offsetHeight;
				this.height=screenH-header-footer;
			//}
	...
	Cell.prototype._configureCell=function () {
		...
		if (self.column.definition.cssClass) {
			var sp=self.column.definition.cssClass.split(" ");
			element.classList.add(sp[0]);
		}
		//set event bindings
	...
	FooterManager.prototype.createElement=function () {
		var el=document.createElement("div");
		if(this.table.options.noFooter){
			el.style.class="";
	...
	Tabulator.prototype.defaultOptions={
		...
		fitToScreen:false,
		maxPage:1,
		columnTitleHeight:0, //height of columns title holder
		topOffset:0,        //if table align by bottom of page, the offset from top of page
			...
			if (data.indexOf("{") == 0 || data.indexOf("[") == 0) {
				...
				if (self.modExists("ajax", true)) {
					...
					if (self.options.pagination == "remote" && self.modExists("page", true)) {
						self.modules.page.reset(true);
						return self.modules.page.setPage(0);
					} else {
						//assume data is url, make ajax call to url to get data
	...
	Ajax.prototype._loadDataProgressive=function () {
		...
		return this.table.modules.page.setPage(0);
	};
			...
			if (url) {
				...
				if(0){		
					...
					xhr.onload=function() {
								...
								var data=formatReport(this.responseText);
								resolve(data);
					}
	...
	Format.prototype.formatters={
		...
		tickCross: function tickCross(cell, formatterParams, onRendered) {
			var value=cell.getValue(),
			    ...
			    tick=typeof formatterParams.tickElement !== "undefined" ? formatterParams.tickElement : '<svg enable-background="new 0 0 24 24" height="14" width="14" viewBox="0 0 24 24" xml:space="preserve" ><path fill="'+barColor+'" clip-rule="evenodd" d="M21.652,3.211c-0.293-0.295-0.77-0.295-1.061,0L9.41,14.34  c-0.293,0.297-0.771,0.297-1.062,0L3.449,9.351C3.304,9.203,3.114,9.13,2.923,9.129C2.73,9.128,2.534,9.201,2.387,9.351  l-2.165,1.946C0.078,11.445,0,11.63,0,11.823c0,0.194,0.078,0.397,0.223,0.544l4.94,5.184c0.292,0.296,0.771,0.776,1.062,1.07  l2.124,2.141c0.292,0.293,0.769,0.293,1.062,0l14.366-14.34c0.293-0.294,0.293-0.777,0-1.071L21.652,3.211z" fill-rule="evenodd"/></svg>',
		...
		buttonTick: function buttonTick(cell, formatterParams, onRendered) {
			return '<svg enable-background="new 0 0 24 24" height="14" width="14" viewBox="0 0 24 24" xml:space="preserve" ><path fill="'+barColor+'" clip-rule="evenodd" d="M21.652,3.211c-0.293-0.295-0.77-0.295-1.061,0L9.41,14.34  c-0.293,0.297-0.771,0.297-1.062,0L3.449,9.351C3.304,9.203,3.114,9.13,2.923,9.129C2.73,9.128,2.534,9.201,2.387,9.351  l-2.165,1.946C0.078,11.445,0,11.63,0,11.823c0,0.194,0.078,0.397,0.223,0.544l4.94,5.184c0.292,0.296,0.771,0.776,1.062,1.07  l2.124,2.141c0.292,0.293,0.769,0.293,1.062,0l14.366-14.34c0.293-0.294,0.293-0.777,0-1.071L21.652,3.211z" fill-rule="evenodd"/></svg>';
		},
		//cross icon
	...
	Page.prototype.createElements=function () {
		...
		var goField=document.createElement("input");
		goField.classList.add("tabulator-field");
		goField.setAttribute("type", "text");
		goField.setAttribute("id", "pageGo");
		goField.setAttribute("size", "7");
		goField.value="0";
		
		var pageSpan=document.createElement("span");
		pageSpan.classList.add("tabulator-span");
		pageSpan.setAttribute("id", "pageSpan");
		
		this.pageSpan=pageSpan.cloneNode(true);
		
		this.goField=goField.cloneNode(true);
		
		this.goBut=button.cloneNode(true);
		this.goBut.setAttribute("data-page", "goPage");
	...
	Page.prototype.initialize=function (hidden) {
		...
		//bind localizations
		self.table.modules.localize.bind("pagination|go", function (value) {
			self.goBut.innerHTML=value;
		});
		
		self.table.modules.localize.bind("pagination|go_title", function (value) {
			self.goBut.setAttribute("aria-label", value);
			self.goBut.setAttribute("title", value);
		});
		...
		//click bindings
		self.goField.addEventListener("keypress", function (e) {
			var currentCode = event.which || event.code;		
  		    if (currentCode == 13 ) { //Enter
				var page=getNode("pageGo").value;
				if(isNaN(page)){
					alert("Page input have to be number")
				}else{
					self.setPage(page); // show page 
				}
			}		
			
		});
		
		self.goBut.addEventListener("click", function () {
			var page=getNode("pageGo").value;
			if(isNaN(page)){
				alert("Page input have to be number")
			}else{
		...
		self.lastBut.addEventListener("click", function () {
			self.setPage(self.max-1);
		});
		if (self.table.options.paginationElement) {
			self.element=self.table.options.paginationElement;
		}
		//append to DOM
		self.element.appendChild(self.pageSpan);
		self.element.appendChild(self.goField);
		self.element.appendChild(self.goBut);
	...
	Page.prototype.setPage=function (page) {
		...
		return new Promise(function (resolve, reject) {
			if (page >= 0 && page < _this33.max) {
				_this33.page=page;
				var t="Page "+page;
				if(_this33.max>1)t+=" from "+_this33.max;
				_this33.pageSpan.textContent=t;   
				_this33.goField.value=_this33.page;
				...
				console.warn("Pagination Error - Requested page is out of range of 0 - " + _this33.max + ":", page);
				reject();
			}
		});
	};
	...
	Page.prototype._setPageButtons=function () {
		...
		var min=this.max - this.page + leftSize < this.count ? this.max - this.count + 1 : Math.max(this.page - leftSize, 0);
	...
	Page.prototype.previousPage=function () {
		...
		return new Promise(function (resolve, reject) {
			if (_this34.page >= 0) {
				...
				var t="Page "+_this34.page;
				if(_this34.max>1)t+=" from "+_this34.max;
				_this34.pageSpan.textContent=t;
				...
				console.warn("Pagination Error - Previous page would be less than page 0:", 0);
				reject();
			}
		});
	};
	...
	Page.prototype.nextPage=function () {
		...
		return new Promise(function (resolve, reject) {
			if (_this35.page < _this35.max) {
				_this35.page++;
				var t="Page "+_this35.page;
				if(_this35.max>1)t+=" from "+_this35.max;
				_this35.pageSpan.textContent=t;
				...
				if (!_this35.progressiveLoad) {
					console.warn("Pagination Error - Next page would be greater than maximum page of " + _this35.max + ":", _this35.max + 0);
				}
				reject();
			}
		});
	...
	Page.prototype._parseRemoteData=function (data) {
		...
		if (data[this.paginationDataReceivedNames.data]) {
			//12345 - set page num from ajax data
			this.max=parseInt(data[this.paginationDataReceivedNames.last_page]) || 0;
			this.table.options.maxPage=this.max;
			
			var t="Page "+this.page;
			t+=" from "+this.max;
			self.pageSpan.textContent=t;   
			self.goField.value=this.page;			
...
function editPageRecord(){
	var pageElement=getNode("pageEdit");
	if(pageElement)pageElement.innerHTML=pageContent;
}
...
window.addEventListener("click", function(event){
	...
	if(showMainMenu){
		if(event.clientX<911&&event.clientY<570){
		}else{
			mainMenuStart();
		}
	}
